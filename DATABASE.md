...
- **`Twin`** — cached, precomputed match between two users (`similarityScore`, `sharedParallelTypeIds`). Computed on-demand (`UsersService.refreshTwinMatches`, called every time `GET /v1/users/twins` is hit — see `API.md` §8), not by a scheduled job as originally planned, since no BullMQ job infrastructure is wired up anywhere in this codebase yet. **Invariant, enforced in code**: `saveTwinMatch()` sorts the pair (`[userAId, userBId].sort()`) before every upsert, so `userAId` is always the lexicographically smaller ID — `@@unique([userAId, userBId])` only blocks an exact duplicate in the same order, so without this the same relationship computed as `(X, Y)` and later `(Y, X)` would insert two rows. Matching itself is a weighted-strength similarity (average closeness of `strengthPct` across each pair's shared _visible_ Parallels), not Jaccard/set-overlap, persisted only above a 70% threshold.
